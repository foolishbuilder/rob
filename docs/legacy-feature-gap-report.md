...
| `robctl` / admin command breadth | Old bot offered larger admin command surface (`!throne status/list/search/addsend/addsub/adddomme`, admin HTTP endpoints). | **Partial** | Yes | P1 | v2 `robctl` now has friendly help and key leaderboard/sends/count paths; some throne commands remain planned placeholders. |
...
| Carl-bot warn relay | Old warn-log listener + courtesy DM in `bot/event_cog.py` (`_process_carlbot_warn_message`). | **Missing** | Needs decision | P2 | Potentially useful moderation helper; requires channel/user configuration and safe parsing. |
| Blacklist operations | Old `rob-blacklist`, `rob-unblacklist`, `throne-blacklist` in `bot/event_cog.py`. | **Partial** | Yes | P1 | v2 has blacklist repository and global interaction guard, but no equivalent admin command set yet. |
| Inactivity removal | Old inactivity loop and removal scheduling in `bot/event_cog.py` (`inactivity_loop`, `_process_inactive_members`). | **Missing** | Needs decision | P2 | High-risk moderation behavior; should be dry-run + explicit policy review before porting. |
...
- **Biggest remaining optional ports** from old Rob are moderation-side utilities (`!rule`, warn relay, DM audit, inactivity automation, richer blacklist admin commands).
