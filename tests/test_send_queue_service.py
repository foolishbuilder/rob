...
class _FakeDommes:
    ...
    async def get_by_user_id(self, _guild_id: int, _discord_user_id: int):
        return SimpleNamespace(notification_mode=self.mode)
...
def test_send_queue_skips_public_post_for_private_modes(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr("rob.services.send_queue_service.discord.TextChannel", _FakeChannel)
    sends = _FakeSends()
    leaderboard = _FakeLeaderboard()
    service = SendQueueService(
