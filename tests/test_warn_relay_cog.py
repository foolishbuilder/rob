...
def test_warn_relay_falls_back_to_first_non_moderator_mention():
    bot = _FakeBot()
    cog = WarnRelayCog(bot)
    embed = discord.Embed(title="Warn | Case #55")
    embed.add_field(name="Moderator", value="<@999>", inline=False)
    embed.add_field(name="Details", value="<@123> broke a rule", inline=False)
    msg = SimpleNamespace(
        id=5001,
        guild=SimpleNamespace(id=42),
        channel=SimpleNamespace(id=777),
...
def test_warn_relay_falls_back_to_description_mention():
    bot = _FakeBot()
    cog = WarnRelayCog(bot)
    embed = discord.Embed(title="Warn | Case #77", description="Warning issued to <@123>")
    msg = SimpleNamespace(
        id=5002,
        guild=SimpleNamespace(id=42),
        channel=SimpleNamespace(id=777),
