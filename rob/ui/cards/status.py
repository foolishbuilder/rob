...
def status_card(*, bot_name: str, database_ok: bool, maintenance: MaintenanceState, queue: QueueStatus):
    ...
    return render(make_card(title=f"{bot_name} | Status", body="Shared PostgreSQL health and queue state.", color=color if database_ok else COLOR_PRIMARY, footer=STATUS_FOOTER, sections=sections, variant="status"))
