...
def leaderboard_personal_stats_card(
    *,
    ...
    if domme_stats is not None:
        domme_body = (
            f"Here is how {domme_stats.display_name} is sitting on the board.\n\n"
            f"🏆 Rank: {_rank_label(domme_stats.rank)}\n"
            f"📬 Sends received: {domme_stats.send_count}\n"
            f"💰 Total received: {format_money_from_cents(domme_stats.total_cents)}\n"
            f"💝 Top sender: {domme_stats.top_sub_label}"
        )
        view.add_item(
            discord.ui.Container(
                discord.ui.TextDisplay(f"## {domme_stats.display_name}'s Stats"),
                discord.ui.Separator(),
        ...
        if domme_stats.latest_amount_cents is None:
            ...
            latest_body = (
                ...
                f"{format_money_with_currency_name(domme_stats.latest_amount_cents, domme_stats.latest_currency or 'USD')} from {domme_stats.latest_sub_label or 'Sub with no nickname claimed'}"
            )
        view.add_item(
            _latest_send_details_section(
                title="Most recent send received",
                body=latest_body,
    ...
    if sub_stats is not None:
        sub_body = (
            f"Here is what Rob has on record for {sub_stats.display_name}.\n\n"
            f"📬 Sends made: {sub_stats.send_count}\n"
            f"💰 Total sent: {format_money_from_cents(sub_stats.total_cents)}\n"
            f"💝 Most sent to: {sub_stats.top_domme_label}"
        )
        view.add_item(
            discord.ui.Container(
                discord.ui.TextDisplay(f"## {sub_stats.display_name}'s Stats"),
                discord.ui.Separator(),
        ...
        if sub_stats.latest_amount_cents is None:
            ...
            latest_sub_body = (
                ...
                f"{format_money_with_currency_name(sub_stats.latest_amount_cents, sub_stats.latest_currency or 'USD')} to {sub_stats.latest_domme_label or 'User not in server or has not connected account'}"
            )
        view.add_item(
            _latest_send_details_section(
                title="Last thing they sent",
